...
export function LenderCard({ a, selected, viewing, onLend, onOpen }: { a: Assessed; selected: boolean; viewing?: boolean; onLend: () => void; onOpen: () => void }) {
  const { c } = a;
  const color = scoreColor(c.latest.score);
  const q = a.provider.qualified;
  return (
    <div role="button" tabIndex={0} onClick={onOpen} onKeyDown={(e) => { if (e.key === "Enter") onOpen(); }} className={`card flex h-full cursor-pointer flex-col p-4 transition-colors hover:bg-panel-2 ${selected ? "ring-1 ring-ink/60" : viewing ? "ring-1 ring-line" : ""}`}>
      ...
      <div className="mt-auto flex items-center justify-between gap-2 border-t border-line-soft pt-3">
        ...
        {selected ? <Pill tone="accent">Presta</Pill> : q ? <button type="button" onClick={(e) => { e.stopPropagation(); onLend(); }} className="h-7 rounded-lg bg-ink px-3 text-[12px] font-medium text-panel hover:bg-ink/90">Prestar</button> : <Pill>No cualifica</Pill>}
      </div>
    </div>
  );
}
