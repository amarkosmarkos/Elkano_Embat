...
export default function Lenders({ initialLender }: { initialLender: string | null }) {
  const { network, error, assessed, lenderId, setLender, setOpenCompany } = useMarketplace();
  ...
  const bubbles = useMemo(() => {
    ...
    // área = tesorería desplegable real (escala raíz, tope en el percentil 95 para que un gigante no aplaste al resto)
  ...
  return (
    <div className="grid grid-cols-1 gap-5 xl:grid-cols-[320px_minmax(0,1fr)]">
      <aside className="flex flex-col gap-4">
        <Card title="Prestamistas cualificados" sub={network ? monthLabelLong(network.asOf) : "cargando…"}>
          {!network ? <Skeleton className="h-24" /> : (
            <>
              ...
              <p className="mt-3 text-[12px] text-ink-mute">Cualifica con score ≥ 75, sin alerta ni alarmas, un año de historia, suelo ≥ 60 en 6 meses, liquidez que no lastre y <span className="text-ink">tesorería desplegable real ≥ 100 k€</span> (50 % del suelo de caja de 12 meses, reconstruida desde los movimientos). Área de la burbuja = tesorería desplegable.</p>
            </>
          )}
        </Card>
        <Card title="Filtros">
          <div className="space-y-3">
            ...
            <div><div className="mb-1.5 text-[13px] text-ink-mute">Orden</div><Seg className="w-full" value={sort} onChange={setSort} options={[{ value: "deployable", label: "Tesorería" }, { value: "capacity", label: "Capacidad" }, { value: "score", label: "Score" }, { value: "momentum", label: "Momentum" }]} /></div>
        ...
        {lender ? (
          <div className="card p-4 ring-1 ring-ink/40">
            <div className="flex items-center justify-between text-[12px] text-ink-mute"><span>Presta como</span><button type="button" onClick={() => setOpenCompany(lender.c.id)} className="hover:text-ink">detalle →</button></div>
      ...
      <Card
        title={view === "cards" ? `${filtered.length} empresas · ${sort === "deployable" ? "más tesorería primero" : "las más fuertes primero"}` : "Toda la red · área = tesorería desplegable real"}
        sub={view === "cards" ? "listas para prestar · pincha el nombre para ver el detalle" : "anillo blanco = cualificada · pincha una burbuja para ver el detalle y elegirla"}
      ...
      >
        {!network ? <Skeleton className="h-[420px]" /> : view === "cards" ? (
          <div className="grid grid-cols-1 gap-4 md:grid-cols-2 2xl:grid-cols-4">
            {pageItems.map((a) => <LenderCard key={a.c.id} a={a} selected={a.c.id === lenderId} onLend={() => setLender(a.c.id)} onOpen={() => setOpenCompany(a.c.id)} />)}
        ...
        ) : <BubbleMap data={bubbles} onSelect={(id) => setOpenCompany(id)} sizeLabel="Desplegable" xThreshold={75} thresholdLabel="UMBRAL PRESTAMISTA 75" />}
      </Card>
    </div>
  );
}
