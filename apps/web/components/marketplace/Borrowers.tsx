          ...
          <>
            ...
            {view === "portfolio" && shown.positions.length > 0 && tab === "crew" && (
              <Card title="Posiciones" sub="ordenadas por rango · pincha para verla en el visor">
                ...
                <div className="divide-y divide-line-soft">{shown.positions.map((p) => <button type="button" key={p.id} onClick={() => setOpenCompany(p.id)} className={`grid w-full grid-cols-[28px_minmax(0,1fr)_56px_64px_64px_70px_90px] items-center gap-2 py-2 text-left text-[13px] hover:bg-panel-2 ${openCompany === p.id ? "bg-panel-2" : ""}`}><span className="num text-ink-mute">{p.rank}</span><span className="min-w-0"><span className="block truncate text-ink">{p.name}</span><span className="num block text-[11px] text-ink-mute">{p.id}</span></span><span className="num text-right font-medium" style={{ color: scoreColor(p.score) }}>{p.score.toFixed(0)}</span><span className="num text-right text-ink">{pct(p.pricing.rate)}</span><span className="num text-right text-ink-dim">{pct(p.pricing.pd, 0)}</span><span className={`num text-right ${p.pricing.netYield >= config.targetReturn ? "text-good" : "text-warn"}`}>{pct(p.pricing.netYield)}</span><span className="num text-right font-medium text-ink">{fmtMoney(p.amount)}</span></button>)}</div>
              </Card>
            )}
          </>
        )}
