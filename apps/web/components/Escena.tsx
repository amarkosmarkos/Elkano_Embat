...
export function Escena({number}:{number:number;windows?:WindowData[]}) {
  ...
  return <div ref={track} className={`scene-track scene-number-${number}`} style={{height:`${autoplay?100:media.height}svh`}}>
    <section className="scene-stage" aria-label={media.title}>
      ...
      {number===6&&<Layer p={error||reduced?1:p} from={.5}><h1 className="story-thanks">Gracias por escuchar.</h1><p className="scene-credits">Luken, Nagore, Markos, David y Xuban</p><p className="scene-eyebrow">HackSpain 2026, Reto X Ray de Embat</p><div className="closing-platform"><Link href="/plataforma/" className="closing-platform-button" tabIndex={error||reduced||p>.5225?0:-1}>Ir a la plataforma <span aria-hidden="true">→</span></Link><p>Explora los datos. Descubre las oportunidades.</p></div></Layer>}
