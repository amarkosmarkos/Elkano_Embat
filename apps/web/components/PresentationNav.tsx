...
export function PresentationNav() {
  ...
  return <nav className={`presentation-nav ${scene&&!paper?"on-scene":"on-product"}`} aria-label="Recorrido de la presentación">
    <div className="presentation-controls">
      ...
      {next?<Link href={`${presentationHref(next[0])}${next[0].startsWith("/caso/")?"&boat=skip":""}`} className="presentation-arrow presentation-skip" title="Siguiente sección sin animación" aria-label={`Saltar sin animación: ${next[1]}`}>»</Link>:<Link href={presentationHref(presentation[0][0])} className="presentation-arrow presentation-skip" aria-label="Volver a la primera sección">»</Link>}
    </div>
