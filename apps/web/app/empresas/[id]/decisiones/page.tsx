...
export default async function DecisionesPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const pool = snap && me ? {
    ...
    proposals: snap.proposals.filter((p) => p.fromId === id || p.toId === id).map((p) => ({ id: p.id, fromId: p.fromId, toId: p.toId, fromName: companyName(store.byId.get(p.fromId)), toName: companyName(store.byId.get(p.toId)), amountEur: p.amountEur, netSavingEur: p.netSavingEur, days: p.days, requiresReview: p.requiresReview })),
    totals: snap.totals,
  } : null;
